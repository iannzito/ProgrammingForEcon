...
variables <- data.frame(
  ...
  Meaning = c(
    ...
    "Annual minimum wage growth",
    "Percentage change in HICP between 2015 and 2025",
...
# Create long format HICP variable
hicp_long <- rent %>%
  pivot_longer(
    cols = matches("^20"),
    names_to = "year",
...
# Create long format minimum wage growth variable
wage_long <- wage %>%
  pivot_longer(
    cols = matches("^20"),
    names_to = "year",
...
# Merge HICP and wage variables into one dataset
analysis <- left_join(
  hicp_long,
  wage_long,
  by = c("TIME", "year")
...
# Create minimum wage status variable
wage_countries <- unique(wage_long$TIME)

subpop <- rent %>%
  mutate(
    min_wage_status = ifelse(
      TIME %in% wage_countries,
      "Has minimum wage",
      "No minimum wage"
    )
  )
